...
<div class="content-wrapper">
   ...
   <section class="content content-library">
      ...
      <?php echo Form::open(['route' => ['physician.question.answers',$questionSet->id],'id'=>'physicianAnswerFrm','name'=>'physicianAnswerFrm']); ?>


      <div class="content-library-bg"></div>
      <?php $total                                      = count($questionSet->question->questionSets) - ($questionSet->question->questionSetsyesNoCount->count()); ?>
      <?php $answered                                   = count($questionSet->answers); ?>       
      <h4 class="pull-left">Questions <span class="txt-blue"><?php echo $total; ?></span></h4>
      <h5 class="pull-right">Patient : <span class="txt-blue"><?php echo e($questionSet->patient->first_name ." ".$questionSet->patient->last_name); ?></span></h5>
      ...
      <div class="content-sub mrgn-btm-20 pdng-0">
       ...
       <?php $pos= 1; $yesnoquestions = []; $questions = [];?>           
        <?php $__currentLoopData = $questionSet->question->questionSets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $masterqSets): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?>            
      ...
      <div class="btn-wrap">  
          <?php if('completed' == $status && hasPermission('medical_records_edit')): ?>
          <button type="submit" class="btn btn-primary mrgn-lft-15 submitButton">Done</button>
          <?php endif; ?>
          <a href="<?php echo route('physician.patient.details',$questionSet->patient->id); ?>"><button type="button" class="btn btn-default mrgn-lft-15" onclick="location.href = '<?php echo route('physician.patient.details',$questionSet->patient->id); ?>'">Cancel</button></a>
