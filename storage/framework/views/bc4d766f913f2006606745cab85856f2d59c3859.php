<?php
$style = [
    /* Layout ------------------------------ */
    'html' => 'min-height: 100%;font-size: 14px;font-family: sans-serif;',
    'body' => 'min-height: 100%;',
    'table' => 'width:100%;',
    'table.content' => 'margin-top: 50px;',
    'table.block1' => 'margin-top: 20px;width:60%;',
    'table.block1.td' => 'padding: 8px 0;',
    'table.block2' => 'line-height: 20px;width:100%;',
    'table.h3' => 'margin-bottom: 0; margin-top: 20px; color: #00a0e3;',
];
?>
<!DOCTYPE html>
<html style="<?php echo e($style['html']); ?>">
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
        <title>Summary Report</title>
    </head>
    <body style="<?php echo e($style['body']); ?>">        
        <table style="<?php echo e($style['table'].$style['table.content']); ?>">
            <tr>
                <td>
                    <img src="data:image/png;base64," alt="logo">
                </td>
            </tr>
        </table>
        <hr>
        <table  style="<?php echo e($style['table'].$style['table.block1']); ?>">
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Patient Name</td><td><?php echo $summaryDet['patient']['first_name']; ?> <?php echo $summaryDet['patient']['last_name']; ?></td>
            </tr>
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Gender</td><td><?php if($summaryDet['patient']['gender'] == 'F' ): ?> <?php echo e('Female'); ?> <?php else: ?><?php echo e('Male'); ?> <?php endif; ?></td>
            </tr>
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Chief Complaint</td><td><?php echo ucwords($summaryDet['question']['title']); ?></td>
            </tr>
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Physician Name</td><td><?php echo $summaryDet['physician']['name']; ?></td>
            </tr>
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Report generated on</td><td><?php echo date('m-d-Y H:i:s'); ?></td>
            </tr>
        </table>
        <hr>
        <table  style="<?php echo e($style['table'].$style['table.block2']); ?>">
            <tr>
                <td><h3 style="<?php echo e($style['table.h3']); ?>">Summary Report</h3></td>
            </tr>
            <tr>
                <td><?php echo e($summary); ?></td>
            </tr>
        </table>
    </body>
</html> 