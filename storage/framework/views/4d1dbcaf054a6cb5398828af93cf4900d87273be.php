<?php
$style = [
    /* Layout ------------------------------ */
    'html' => 'min-height: 100%;font-size: 14px;font-family: sans-serif;',
    'body' => 'min-height: 100%;',
    'table' => 'width:100%;',
    'table.content' => 'margin-top: 50px;',
    'table.block1' => 'margin-top: 20px;width:60%;',
    'table.block1.td' => 'padding: 6px 0;',
    'table.block2' => 'line-height: 20px;width:100%;',
    'table.h3' => 'margin-bottom: 0; margin-top: 20px; color: #00a0e3;',
    'remove.margin' => 'margin: 0;',
    'margin.btm.ten' => 'margin-bottom: 10px;',
];
?>
<!DOCTYPE html>
<html style="<?php echo e($style['html']); ?>">
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
        <title>Summary Report</title>
    </head>
    <body style="<?php echo e($style['body']); ?>">
        <table style="<?php echo e($style['table'].$style['table.content']); ?>">
            <tr>
                <td>
                    <img src="data:image/png;base64," alt="logo">
                </td>
            </tr>
        </table>
        <hr>
        <table  style="<?php echo e($style['table'].$style['table.block1']); ?>">
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Patient Name</td><td><?php echo e($evaluationData['patient']['first_name']. " ". $evaluationData['patient']['last_name']); ?></td>
            </tr>
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Age</td><td><?php echo e(Carbon\Carbon::parse($evaluationData['patient']['dob'])->diff(Carbon\Carbon::now())->format('%y')); ?></td>
            </tr>
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Gender</td><td><?php if($evaluationData['patient']['gender'] == 'F' ): ?> <?php echo e('Female'); ?> <?php else: ?><?php echo e('Male'); ?> <?php endif; ?></td>
            </tr>
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Chief Complaint</td><td><?php echo e($evaluationData['question']['title']); ?></td>
            </tr>
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Physician Name</td><td><?php echo e(Auth::user()->name); ?></td>
            </tr>
            <tr>
                <td style="<?php echo e($style['table.block1.td']); ?>">Report generated on</td><td><?php echo date('m-d-Y H:i:s'); ?></td>
            </tr>
        </table>
        <hr>
        <table  style="<?php echo e($style['table'].$style['table.block2']); ?>">
            <tr>
                <td><h3 style="<?php echo e($style['table.h3']); ?>"><?php echo e($evaluationData['question']['title']); ?></h3></td>
            </tr>
            <tr>
                <td><?php echo e($summary); ?></td>
            </tr>
        </table>
        <br>
        <hr>

        <table  style="<?php echo e($style['table'].$style['table.block2']); ?>">
            <tr>
                <td><h3 style="<?php echo e($style['table.h3']); ?>">Allergies</h3></td>
            </tr>
            <?php $__empty_1 = true; $__currentLoopData = $medicalHistoryDetails['patient_allergies']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $medicalHistory): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); $__empty_1 = false; ?>
            <tr>
                <td>
                    <p>
                        <?php echo e(++$key); ?> . <?php echo $medicalHistory['type']; ?>

                        <br>
                        <?php echo $medicalHistory['description']; ?>

                    </p>
                </td>
            </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); if ($__empty_1): ?>
            <tr>
                <td>
                    <p>No Record</p>
                </td>
            </tr>
            <?php endif; ?>
        </table>
        
        <table  style="<?php echo e($style['table'].$style['table.block2']); ?>">
            <tr>
                <td><h3 style="<?php echo e($style['table.h3']); ?>">Medications</h3></td>
            </tr>
            <?php $__empty_1 = true; $__currentLoopData = $medicalHistoryDetails['medications']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $medication): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); $__empty_1 = false; ?>
            <tr>
                <td>
                    <p>
                        <?php echo e(++$key); ?> . <?php echo $medication['type']; ?>

                        <br>
                        <?php echo $medication['description']; ?>

                    </p>
                </td>
            </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); if ($__empty_1): ?>
            <tr>
                <td>
                    <p>No Record</p>
                </td>
            </tr>
            <?php endif; ?>
        </table>

        <table  style="<?php echo e($style['table'].$style['table.block2']); ?>">
            <tr>
                <td><h3 style="<?php echo e($style['table.h3']); ?>">Past Medical History</h3></td>
            </tr>
            <?php $__empty_1 = true; $__currentLoopData = $medicalHistoryDetails['past_medical_history']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $medicalHistory): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); $__empty_1 = false; ?> 
            <tr>
                <td>
                    <p>
                        <?php echo e(++$key); ?> . <?php echo $medicalHistory['type']; ?>

                        <br>
                        <?php echo $medicalHistory['description']; ?>

                    </p>
                </td>
            </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); if ($__empty_1): ?>
            <tr>
                <td>
                    <p>No Record</p>
                </td>
            </tr>
            <?php endif; ?>
        </table>
        <table  style="<?php echo e($style['table'].$style['table.block2']); ?>">
            <tr>
                <td><h3 style="<?php echo e($style['table.h3']); ?>">Past Surgical History</h3></td>
            </tr>
            <?php $__empty_1 = true; $__currentLoopData = $medicalHistoryDetails['surgical_history']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $medicalHistory): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); $__empty_1 = false; ?> 


            <tr>
                <td>
                    <p style="<?php echo e($style['remove.margin']); ?>">
                        <?php echo e(++$key); ?> . <b><?php echo $medicalHistory['surgery']; ?></b> done on <?php echo Carbon\Carbon::parse($medicalHistory['surgery_date'])->format('m-d-Y'); ?>

                    </p>
                </td>
            </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); if ($__empty_1): ?>
            <tr>
                <td>
                    <p>No Record</p>
                </td>
            </tr>
            <?php endif; ?>
        </table>

        <table  style="<?php echo e($style['table'].$style['table.block2']); ?>">
            <tr>
                <td><h3 style="<?php echo e($style['table.h3']); ?>">Family History</h3></td>
            </tr>
            <?php $__empty_1 = true; $__currentLoopData = $medicalHistoryDetails['family_history']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $medicalHistory): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); $__empty_1 = false; ?>             
            <tr>
                <td>
                    <p style="<?php echo e($style['margin.btm.ten']); ?>">
                        <?php echo e(++$key); ?> . <?php echo $medicalHistory['relation']; ?>

                        <br>
                        <?php echo $medicalHistory['illness']; ?>

                    </p>
                </td>
            </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); if ($__empty_1): ?>
            <tr>
                <td>
                    <p>No Record</p>
                </td>
            </tr>
            <?php endif; ?>
        </table>

        <table  style="<?php echo e($style['table'].$style['table.block2']); ?>">
            <tr>
                <td><h3 style="<?php echo e($style['table.h3']); ?>">Social History</h3></td>
            </tr>
            <?php $__currentLoopData = $medicalHistoryDetails['social_history']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $medicalHistory): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?> 

            <tr>
                <td>Smoking - <?php if($medicalHistory['smoke']): ?> <?php echo e("Yes"); ?> <?php else: ?> <?php echo e("No"); ?> <?php endif; ?></td>
            </tr>
            <tr>
                <td>Drinking - <?php if($medicalHistory['drink']): ?> <?php echo e("Yes"); ?> <?php else: ?> <?php echo e("No"); ?> <?php endif; ?></td>
            </tr>
            <tr>
                <td>Drug - <?php if($medicalHistory['drug']): ?> <?php echo e("Yes"); ?> <?php else: ?> <?php echo e("No"); ?> <?php endif; ?></td>
            </tr>
            <tr>
                <td>
                    <h3 style="<?php echo e($style['table.h3']); ?>">Other Comments</h3>
                    <br>
                    <?php echo $medicalHistory['comments']; ?></td>
            </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); ?>
        </table>
        <?php if($clinicalReport): ?>
        <table  style="<?php echo e($style['table'].$style['table.block2']); ?>">
            <tr>
                <td><h3 style="<?php echo e($style['table.h3']); ?>">Clinical Question information</h3></td>
            </tr>
            <?php $__currentLoopData = $clinicalReport; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $clinicalQs=>$clinicalAns): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?> 

            <tr>
                <td><?php echo $loop->iteration; ?> . <?php echo $clinicalQs; ?></td>
            </tr>
            <tr>
                <td><?php echo $clinicalAns; ?></td>
            </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); ?>
        </table>
        <?php endif; ?>
    </body>
</html> 