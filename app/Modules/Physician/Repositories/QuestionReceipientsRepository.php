...
// creating the new patient entry  (a dummy registration) . getting the user id to map to the question set
                $resultData                = $this->patient->create(array('first_name' => '-', 'email' => str_random(32), 'contact_number' => $inputData['phone'], 'password' => str_random(32), 'is_account_active' => 'P', 'gender' => '-', 'entry_type' => 'T'));
        ...
        if (count($checkIfRespondedActive) > 0)
            $this->updateQuestionSetsAsPending($checkIfRespondedActive->id);

        if (count($checkIfSetActive) == 0 && count($checkIfRespondedActive) == 0) {
