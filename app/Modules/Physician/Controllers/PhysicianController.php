...
// get question set
                    ...
                    ->editColumn('title', function($questionSets) use ($requestData) {
                        ...
                        if (key_exists('showUseButton', $requestData) && $requestData['showUseButton'] == '1') {
                            $return .= '<button type="button" class="btn btn-third btn-block mrgn-btm-15" title="Preview"
                                        onclick="location.href =\'' . url('physician/questionSetPreview/' . $questionSets->id . '/create-set') . '\'">Preview</button>';
                            if (Auth::user()->isAuthorizedStaff('')) {
                                $return .='<button type="button" class="btn btn-third btn-block"  title="Send"
                                            onclick="location.href= \'' . url('physician/use-the-question-set/' . $questionSets->id) . '\'">Use this Question Set</button>';
                            }
                        } else {
                            // listing the published quetsion sets
                            $return .= '<button type="button" class="btn btn-third btn-block mrgn-btm-15" title="Preview"
                                        onclick="location.href =\'' . url('physician/questionSetPreview/' . $questionSets->id . '/published-list') . '\'">Preview</button>';
                            if (Auth::user()->isAuthorizedStaff('questionset_send')) {
                                $return .= '<button type="button" class="btn btn-third btn-block"  title="Send"
                                        onclick="location.href = \'' . url('physician/sendQuestionSet/' . $questionSets->id) . '\'">Send</button>';
                            }
                        }
                        $return .= ' </div>
                                </div>';
    ...
    { //get session data
        ...
        if ($questionSets->user_id != $userId) {
            ...
            return redirect('/physician/sendQuestionSet/' . $newQuestion->id);
        }
    ...
    {
        $requestData                 = $request->all();
        $requestData['physician_id'] = Auth::user()->id;
...
// flag for  sending the question set for the first time
        ...
        if ($requestData['selectType'] == 'single') {
            $requestData['physician_id'] = Auth::user()->getUserId();
            ...
            $inputData['physician_id']  = Auth::user()->getUserId();
            ;
                        ...
                        if ($requestData['listType'] == 'Admin') {
                           ...
                           <div id="collapse' . $notications->id . '" class="panel-collapse collapse">
                            ...
                            if ($notications->is_seen)
                                ...
                                $return .= '<a href="' . url('physician/patients/' . $notications->question_recipients_id . '/questionset') . '" >' . $notications->title . '</a></b> Question Set.';
