...
//            ->where('patients.is_account_active', '=', 'Y')->whereNull('patients.activation_code')
        ...
        return Datatables::of($patients)
                ->add_column('first_name', function($patients) {
                    if (Auth::user()->isAuthorizedStaff('patient_edit')) {
                        if ($patients->is_account_active == 'Y')
                            ...
                            return "<a href='" . route('physician.patient.details', $patients->id) . "'>" . trans("Physician::messages.span_registration_pending") . "</span></a>";
                    }
                    else {
                        if ($patients->is_account_active == 'Y')
                ...
                ->add_column('last_name', function($patients) {
                    ...
                    if (!empty($patients->last_name)) {
                        if (Auth::user()->isAuthorizedStaff('patient_edit'))
                            $result = "<a href='" . route('physician.patient.details', $patients->id) . "'>$patients->last_name</a>";
                        else
    ...
    public function getQuestionSetList(Request $request)
    {
        ...
        $patientId   = $request->id;
        $physicianId = Auth::user()->getUserId();
        ...
        return Datatables::of($patientDetails)
                ->add_column('title', function($patientDetails) {
                    ...
                    if (Auth::user()->isAuthorizedStaff('questionset_list'))
                        return "<a href=" . route('physician.patient.questionsetdetail', $patientDetails->qResId) . ">" . $patientName . "</a>";
                    else
                        return $patientName;
                })
...
//                    if ('pending' == $patientDetails->status)
                ...
                ->add_column('reports', function($patientDetails) {
                    if ('completed' == $patientDetails->status) {
                        ...
                        if (Auth::user()->isAuthorizedStaff('patient_summaryReportList'))
                            $reportLinks .= '<a onclick="return checkClinicalQuestions(' . $patientDetails->question_id . ',' . $patientDetails->qResId . ', \'S\')" href=' . route("physician.patient.summary", $patientDetails->qResId) . ' class=\"txt-blue\">Summary Report</a><br>';
                        if (Auth::user()->isAuthorizedStaff('patient_evaluationReportList'))
                            $reportLinks .= '  <a onclick="return checkClinicalQuestions(' . $patientDetails->question_id . ',' . $patientDetails->qResId . ', \'E\')" href=' . route("physician.patient.evaluation", $patientDetails->qResId) . ' class=\"txt-blue mrgn-lft-25\">Full Evaluation Report</a>';
                        return $reportLinks;
                    }
                })
                ->make(true);
    ...
    public function sendSummaryReport(SendSummaryRequest $request, SummaryReportsRepository $summaryReportsRepo, PhysicianRepository $physicianRepo, NotificationsRepository $notificationsRepo, QuestionReceipientsRepository $questionReceipientRepo)
    {
        ...
        $requestData['phyId']       = Auth::user()->getUserId();
...
//            $sendToId     = $newPhysician->id;
    ...
    public function postAnswer(Request $request)
    {
        ...
        return json_encode(array('success' => true, 'message' => $succMsg, 'redirectUrl' => route('physician.patient.questionsetdetail', $qRecId)));
    }
